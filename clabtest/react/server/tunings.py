from tab_generator import Note
...
def generate_E_standard():
    """
    Generates a dictionary showing all the notes that can be played
    ...
    for x in range(2, 6):
        tunings_map["E standard"].append(Note("Gb", x))
    ...
    for x in range(2, 6):
        tunings_map["E standard"].append(Note("Ab", x))
    ...
    for x in range(2, 6):
        tunings_map["E standard"].append(Note("Bb", x))
    ...
    for x in range(3, 7):
        tunings_map["E standard"].append(Note("Db", x))
    ...
    for x in range(3, 7):
        tunings_map["E standard"].append(Note("Eb", x))
...
def generate_tunings_map():
    """
    Generates a dictionary showing all the notes that can be played
    in a particular tuning
    """
    tunings_map = generate_E_standard()
    
    # Add subsequent tunings based on E standard
    # NOTE: copy.copy must be used to prevent 
    #       notes being added to E standard, Eb standard
    #       etc., when basing the notes in a tuninig off
    #       of the previous tunings.
    
    # add Eb Standard
    e_flat_standard_notes = copy.copy(tunings_map["E standard"])
    e_flat_standard_notes.remove(Note("E", 6))
    e_flat_standard_notes.append(Note("D#", 2))
    e_flat_standard_notes.append(Note("Eb", 2))
    tunings_map["Eb standard"] = e_flat_standard_notes
    
    # add Drop D
    drop_d_notes = copy.copy(tunings_map["E standard"])
    drop_d_notes.append(Note("D", 2))
    drop_d_notes.append(Note("D#", 2))
    drop_d_notes.append(Note("Eb", 2)) 
    tunings_map["Drop D"] = drop_d_notes
   
    # add D standard
    d_standard_notes = copy.copy(tunings_map["Eb standard"])
    d_standard_notes.remove(Note("D#", 6))
    d_standard_notes.remove(Note("Eb", 6))
    d_standard_notes.append(Note("D", 2))
    tunings_map["D standard"] = d_standard_notes
    
    # add C# standard
    c_sharp_standard_notes = copy.copy(tunings_map["D standard"])
    c_sharp_standard_notes.remove(Note("D", 6))
    c_sharp_standard_notes.append(Note("C#", 2))
    c_sharp_standard_notes.append(Note("Db", 2))
    tunings_map["C# standard"] = c_sharp_standard_notes
    
    # add Drop C
    drop_c_notes = copy.copy(tunings_map["D standard"])
    drop_c_notes.append(Note("C", 2))
    drop_c_notes.append(Note("C#", 2))
    drop_c_notes.append(Note("Db", 2))
    tunings_map["Drop C"] = drop_c_notes
    
    # add C standard
    c_standard_notes = copy.copy(tunings_map["C# standard"])
    c_standard_notes.remove(Note("C#", 6))
    c_standard_notes.append(Note("C", 2))
    tunings_map["C standard"] = c_standard_notes
    
    # add Drop B
    drop_b_notes = copy.copy(tunings_map["C standard"])
    drop_b_notes.append(Note("B", 1))
    tunings_map["Drop B"] = drop_b_notes
    
    # add B standard -- start of where guitar is now 7 string
    b_standard_notes = [Note("B", 1), Note("C", 2), Note("C#", 2), Note("D", 2), Note("D#", 2)]
    ...
    # add drop A
    drop_a_notes = copy.copy(tunings_map["B standard"])
    drop_a_notes.append(Note("A", 1))
    drop_a_notes.append(Note("A#", 1))
    drop_a_notes.append(Note("Bb", 1))
...
if __name__ == "__main__":
    ...
    #print("Notes in Eb standard:")
    #for note in tunings["Eb standard"]:
        #print(str(note))
     
    #print(Note("D", -1) in tunings["Eb standard"])    
